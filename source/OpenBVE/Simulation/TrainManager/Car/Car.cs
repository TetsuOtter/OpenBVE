					...
					case CarSectionType.Interior:
						if (this.HasInteriorView && this.CarSections.Length > 0)
						{
							...
							for (int j = 0; j < CarSections[0].Elements.Length; j++)
							{
								...
								{
									OpenBveShared.Renderer.ShowObject(o, ObjectType.Overlay, Interface.CurrentOptions.TransparencyMode, Camera.CameraRestriction != CameraRestrictionMode.NotAvailable);
								}
								else
								{
									OpenBveShared.Renderer.ShowObject(o, ObjectType.Dynamic, Interface.CurrentOptions.TransparencyMode, Camera.CameraRestriction != CameraRestrictionMode.NotAvailable);
								}
							}
							break;
						}
