...
namespace OpenBve
{
	internal static partial class Renderer
	{
		...
		internal static void RenderScene(double TimeElapsed)
		{
			...
			GL.PushMatrix();
			UpdateViewport(OpenBveShared.Renderer.ViewPortChangeMode.ChangeToScenery);
			...
			GL.LoadIdentity();
			UpdateViewport(OpenBveShared.Renderer.ViewPortChangeMode.ChangeToCab);
			...
			if (Camera.CameraRestriction == CameraRestrictionMode.NotAvailable)
			{
				...
				{
					RenderFace(ref OpenBveShared.Renderer.OverlayOpaque.Faces[i], cx, cy, cz);
				}
				// overlay alpha
				...
				if (Interface.CurrentOptions.TransparencyMode == TransparencyMode.Performance)
				{
					...
					{
						RenderFace(ref OpenBveShared.Renderer.OverlayAlpha.Faces[i], cx, cy, cz);
					}
				}
				else
				{
