namespace OpenBveApi.Objects
{
	...
	public partial class Glow
	{
		...
		public static ushort GetAttenuationData(double HalfDistance, AttenuationMode Mode) {
			if (HalfDistance <= 0.0 | Mode == AttenuationMode.None) return 0;
			if (HalfDistance < 1.0) {
				HalfDistance = 1.0;
		...
		public static void SplitAttenuationData(ushort Data, out AttenuationMode Mode, out double HalfDistance) {
			Mode = (AttenuationMode)(Data >> 12);
		...
		public static double GetDistanceFactor(VertexTemplate[] Vertices, ref MeshFace Face, ushort GlowAttenuationData, double CameraX, double CameraY, double CameraZ)
		{
			...
			switch (mode)
			{
				case AttenuationMode.DivisionExponent2:
				{
					double t = dx * dx + dy * dy + dz * dz;
					return t / (t + halfdistance * halfdistance);
				}
				case AttenuationMode.DivisionExponent4:
				{
					double t = dx * dx + dy * dy + dz * dz;
					t *= t;
