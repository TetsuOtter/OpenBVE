...
namespace OpenBve {
	internal static class Program {
		...
		internal static void UpdateViewport() {
			...
			OpenBveShared.World.AspectRatio = (double)Renderer.ScreenWidth / (double)Renderer.ScreenHeight;
			OpenBveShared.World.HorizontalViewingAngle = 2.0 * Math.Atan(Math.Tan(0.5 * OpenBveShared.World.VerticalViewingAngle) * OpenBveShared.World.AspectRatio);
            GL.MatrixMode(MatrixMode.Projection);
            Matrix4d perspective = Matrix4d.CreatePerspectiveFieldOfView(OpenBveShared.World.VerticalViewingAngle, OpenBveShared.World.AspectRatio, 0.2, 1000.0);
