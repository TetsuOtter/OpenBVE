...
namespace OpenBve
{
    ...
    {      
        internal class AnimatedObject : AbstractAnimatedObject
        {
			...
			internal void Initialize(int StateIndex, bool Overlay, bool Show)
			{
				...
				if (Show)
				{
					if (Overlay)
					{
						OpenBveShared.Renderer.ShowObject(i, ObjectType.Overlay, Interface.CurrentOptions.TransparencyMode, OpenBveShared.Camera.CameraRestriction != CameraRestrictionMode.NotAvailable);
					}
					else
					{
						OpenBveShared.Renderer.ShowObject(i, ObjectType.Dynamic, Interface.CurrentOptions.TransparencyMode, OpenBveShared.Camera.CameraRestriction != CameraRestrictionMode.NotAvailable);
					}
				}
			}
			...
			internal void Update(bool IsPartOfTrain, Train Train, int CarIndex, int SectionIndex, double TrackPosition, Vector3 Position, Vector3 Direction, Vector3 Up, Vector3 Side, bool Overlay, bool UpdateFunctions, bool Show, double TimeElapsed, bool EnableDamping)
			{
				...
				for (int k = 0; k < States[s].Object.Mesh.Vertices.Length; k++)
				{
					...
					// translate
					if (Overlay & OpenBveShared.Camera.CameraRestriction != CameraRestrictionMode.NotAvailable)
					{
						...
						double dx = -Math.Tan(OpenBveShared.Camera.CameraCurrentAlignment.Yaw) - OpenBveShared.Camera.CameraCurrentAlignment.Position.X;
						double dy = -Math.Tan(OpenBveShared.Camera.CameraCurrentAlignment.Pitch) - OpenBveShared.Camera.CameraCurrentAlignment.Position.Y;
						double dz = -OpenBveShared.Camera.CameraCurrentAlignment.Position.Z;
        ...
        internal class AnimatedWorldObject : WorldObject
        {
	        ...
	        public override void Update(double TimeElapsed, bool ForceUpdate)
	        {
				...
				if (visible | ForceUpdate)
				{
					...
					if (!Visible)
					{
						OpenBveShared.Renderer.ShowObject(Object.ObjectIndex, ObjectType.Dynamic, Interface.CurrentOptions.TransparencyMode, OpenBveShared.Camera.CameraRestriction != CameraRestrictionMode.NotAvailable);
						Visible = true;
					}
				}
				else
        ...
        internal static int CreateAnimatedWorldObject(AnimatedObject Prototype, Vector3 Position, Transformation BaseTransformation, Transformation AuxTransformation, int SectionIndex, double TrackPosition, double Brightness)
        {
	        ...
	        o.Object.ObjectIndex = GameObjectManager.CreateDynamicObject(Program.CurrentHost);
        ...
        {
            for (int i = 0; i < GameObjectManager.AnimatedWorldObjectsUsed; i++)
            {
                GameObjectManager.AnimatedWorldObjects[i].Update(TimeElapsed, ForceUpdate);
            }
        }
        ...
        internal static void InitializeVisibility()
        {
            ...
            {
                if (GameObjectManager.Objects[i] != null && !GameObjectManager.Objects[i].Dynamic)
                {
            ...
            for (int i = 0; i < GameObjectManager.ObjectsUsed; i++)
            {
                if (GameObjectManager.Objects[i] != null && !GameObjectManager.Objects[i].Dynamic)
                {
                    ...
                    {
                        OpenBveShared.Renderer.ShowObject(i, ObjectType.Static, Interface.CurrentOptions.TransparencyMode, OpenBveShared.Camera.CameraRestriction != CameraRestrictionMode.NotAvailable);
                    }
                }
            }
        }
            ...
            if (d < 0.0)
            {
                ...
                while (GameObjectManager.ObjectsSortedByEndPointer >= 0)
                {
                    ...
                    if (GameObjectManager.Objects[o].EndingDistance >= p - OpenBveShared.World.BackwardViewingDistance)
                    {
                        if (GameObjectManager.Objects[o].StartingDistance <= p + OpenBveShared.World.ForwardViewingDistance)
                        {
                            OpenBveShared.Renderer.ShowObject(o, ObjectType.Static, Interface.CurrentOptions.TransparencyMode, OpenBveShared.Camera.CameraRestriction != CameraRestrictionMode.NotAvailable);
                        }
